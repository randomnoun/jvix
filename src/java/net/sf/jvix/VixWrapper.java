...
public class VixWrapper {
   ...
   * 
    ...
    call VixVM_LoginInGuest() with VIX_CONSOLE_USER_NAME as the userName
    and NULL as the password before calling this function. This will ensure that the 
    ...
    interface, you can pass VIX_RUNPROGRAM_ACTIVATE_WINDOW as the value of the options
    parameter. This option will ensure that the application's window is visible and not
    minimized on the guest's screen. This can be combined with the
    VIX_RUNPROGRAM_RETURN_IMMEDIATELY flag using the bitwise inclusive OR operator (|).
    VIX_RUNPROGRAM_ACTIVATE_WINDOW has no effect on Linux guest operating systems.
   ...
   * <ul><li>This function permanently deletes a virtual machine from your host system. 
     ...
     * <li>Calling VixVM_Delete() on a virtual machine handle does not release the virtual machine handle. 
     You still need to call Vix_ReleaseHandle() on the virtual machine handle.
     * </ul> 
   *  
   * @param vmHandle Identifies a virtual machine. Call VixVM_Open() to create a virtual machine handle.
   ...
   * 
   * <ul><li>Shared folders are indexed from 0 to <i>n</i>-1, where <i>n</i> is the number of shared folders. 
     Use the function VixVM_GetNumSharedFolders() to get the value of <i>n</i>. 
